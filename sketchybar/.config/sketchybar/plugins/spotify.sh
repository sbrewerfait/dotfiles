...
update_track() {
    ...
    if [[ -z $SPOTIFY_JSON ]]; then
        sketchybar --set $NAME icon.color="$ACCENT_COLOR" label.drawing=no
        return
    fi
    ...
    if [ $PLAYER_STATE = "Playing" ]; then
        ...
        sketchybar --set $NAME label="${TRACK}  ${ARTIST}" label.drawing=yes icon.color="$ACCENT_COLOR"
...
#
#     if [[ -z $SPOTIFY_JSON ]]; then
#         echo "Empty SPOTIFY_JSON $NAME"
#         sketchybar --set $NAME icon= icon.color="$ACCENT_COLOR" label.drawing=no
#         return
#     fi
#
#     PLAYER_STATE=$(echo "$SPOTIFY_JSON" | jq -r '.["Player State"]')
...
#             fi
#         fi
#         sketchybar --set $NAME icon= label="${TRACK}  ${ARTIST}" label.drawing=yes icon.color="$ACCENT_COLOR"
#
#     elif [ $PLAYER_STATE = "Paused" ]; then
